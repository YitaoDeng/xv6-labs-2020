...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
  ...
  if(newsz < oldsz)
    return oldsz;
	// user virtual address range not exceed PLIC
	if(newsz >= PLIC)
		return 0;
...
int
kvmcopyuvm(pagetable_t p, pagetable_t kp, uint64 start, uint64 sz)
{
	pte_t *pte, *kpte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = PGROUNDUP(start); i < start + sz; i += PGSIZE){
    ...
    if((*pte & PTE_V) == 0){
		...
		if((kpte = walk(kp, i, 1)) == 0)
			panic("kvmcopy: kpte should exist");
    pa = PTE2PA(*pte);
		// kernal cannot access pages with PTE_U
...
void pageprint(pagetable_t pagetable, int level) 
{
	for (int i = 0; i < 512; i++) {
		pte_t pte = pagetable[i];
		if (pte & PTE_V) {
